...
namespace lab_04
{
    partial class Grammar
    {
        ...
        public bool Compile(string filename, out List<Rule> leftGeneration, out AST ast)
        {
            GenerateSyntaxTable();
            return Analyse(ParseFile(filename), out leftGeneration, out ast);
        }
        ...
        public bool Analyse(List<string> inputStream, out List<Rule> leftGeneration, out AST ast)
        {
            leftGeneration = [];
            Stack<string> stack = [];
            stack.Push("$");
            ...
            ast = new(new Node(Start, []));
            treeStack.Push(ast.Tree);
                ...
                else
                {
                    ...
                    if (syntaxTable[X][a][0] != "e")
                    {
                        for (int i = syntaxTable[X][a].Count - 1; i >= 0; --i)
                        {
                            ...
                            // создание АСД
                            Node childNode = new(syntaxTable[X][a][i]);
                        ...
                        // создание АСД 
                        parentNode.Childs.Add(new Node("e"));
                    // создание АСД
